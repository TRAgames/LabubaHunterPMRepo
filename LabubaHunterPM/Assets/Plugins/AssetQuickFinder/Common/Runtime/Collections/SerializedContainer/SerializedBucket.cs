...
//don't support unsigined integer type
...
namespace QuickFinder.Container
{
    public class SerializedBucket
    {
        ...
        private kint capacity;
        private kint count;
        private kint freeList;
        private uint mode;
        private byte[] bytes;
        ...
        {
            capacity = (kint)HashHelpers.GetPrime(estimateCapacity);
        ...
        public bool Load(byte[] buffer, int istart)
        {
            ...
            BUFFER_START = BOUNDARY_START + HEAD_SIZE;
            BUFFER_LENGTH = capacity * SLOT_SIZE;
            BOUNDARY_LENGTH = BUFFER_START + BUFFER_LENGTH;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Find(kint key, vint value)
        {
            return Find(key, value, out _);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool Find(kint key, vint value, out kint location)
        {
            var slots = GetReadonlySpan();
            var islot = (kint)(key % capacity);
            ...
            for (; ; )
            {
                ...
                if (slots[current].key == key && slots[current].value == value)
                {
                    location = current;
                    return true;
                }
                current = (kint)(slots[current].next & SLOT_MASK);
                if (current == TAIL_SLOT || current == INVALID_SLOT)
                {
                    location = islot;
                    return false;
                }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool FindFirst(kint key, out vint value)
        {
            return FindFirst(key, out value, out _);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool FindFirst(kint key, out vint value, out kint location)
        {
            var slots = GetReadonlySpan();
            var islot = (kint)(key % capacity);
            //var islot = (int)(((int)key[HASH_INDEX] & SLOT_MASK) % capacity);
            ...
            for (; ; )
            {
                ...
                {
                    value = slots[current].value; location = current;
                    return true;
                }
                current = (kint)(slots[current].next & SLOT_MASK);
                if (current == TAIL_SLOT || current == INVALID_SLOT)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool ContainKey(kint key)
        {
            return ContainKey(key, out _);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool ContainKey(kint key, out kint firstLocation)
        {
            var slots = GetReadonlySpan();
            var islot = (kint)(key % capacity);
            //var islot = (int)(((int)key[HASH_INDEX] & SLOT_MASK) % capacity);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public kint Next(kint startLocation, out vint currentValue)
        {
            var slots = GetReadonlySpan();
            return Next(slots, startLocation, out currentValue, out _);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public kint Next(ReadOnlySpan<Slot> slots, kint startLocation, out vint currentValue, out kint currentKey)
        {
            if (startLocation == INVALID_SLOT || startLocation == TAIL_SLOT)
            { currentValue = 0; currentKey = 0; return TAIL_SLOT; }

            currentKey = slots[startLocation].key;
            currentValue = slots[startLocation].value;
            var next = slots[startLocation].next;
            if (next == TAIL_SLOT || next == INVALID_SLOT)
            { return TAIL_SLOT; }

            return (kint)(next & SLOT_MASK);
        }
        ...
        {
            var slots = GetSpan();
            if (Find(key, value, out var islot))
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Delete(kint key, vint value)
        {
            var slots = GetSpan();
            if (!Find(key, value, out var islot))
            { return false; }
        ...
        //INTEGERBUCKET_DELETE_KEY_WITH_VALUE:
            TryAddCount(-1);

            if (slots[current].IsHead())
            {
                if (slots[current].next == HEAD_ONLY_SLOT)
                {
                    PushFree(slots, current);
                }
                else
                {
                    var next = (kint)(slots[current].next & SLOT_MASK);
                    slots[current] = slots[next];
                    slots[current].next = (kint)(slots[next].next + SET_HEAD_MASK);
            ...
            //    var next = Next(current, out var target);
            //    if (target == value)
            //    {
            //        goto INTEGERBUCKET_DELETE_KEY_WITH_VALUE;
            //    }
            //    if (next == TAIL_SLOT)
            //    { break; }
            //}
        ...
        public bool Delete(kint key)
        {
            var slots = GetSpan();
            if (!FindFirst(key, out var value, out var islot))
            { return false; }

            var current = islot;
            for(; ; )
            {
                var next = Next(slots, current, out var currentValue, out var currentKey);
                if (currentKey == key)
                {
                    TryAddCount(-1);
        ...
        public bool Resize()
        {
            ...
            var newCapacity = (kint)HashHelpers.GetPrime(capacity + capacity / 4);
        ...
        public bool Resize(byte[] newbuffer, long istart, long boundaryLength)
        {
            ...
            for (kint i = 0; i < capacity;)
            {
                ...
                var inewSlot = (kint)(slots[i].key % newCapacity);
                ...
                {
                    var inewSlotKeySlot = slots[inewSlot].key % newCapacity;
        ...
        {
            var head = (kint)(slots[islot].key % capacity);
            var next = head;
                ...
                for (; ; )
                {
                    ...
                    if (next - i > 8)
                    {
                        for (kint k = lastk; k < lastk + 8; k++)
                        {
                            ...
                            var kKeySlot = slots[k].key % capacity;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ValueEnumerator EveryValueOf(kint key)
        {
            return new ValueEnumerator(this, key);
        }

        public ref struct ValueEnumerator
        {
            SerializedBucket container;
            ReadOnlySpan<Slot> slots;
            private kint islot;

            public ValueEnumerator(SerializedBucket container, kint key)
            {
                this.container = container;
                slots = container.GetReadonlySpan();
                if (!container.ContainKey(key, out islot))
                { islot = TAIL_SLOT; }
            }
            ...
            public vint Current
            {
                get
                {
                    islot = container.Next(slots, islot, out var value, out _);
                    return value;
                }
            }
            ...
            {
                return container != null && islot != TAIL_SLOT;
            }
